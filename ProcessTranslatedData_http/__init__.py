...
def get_dataverse_token():
    ...
    if (
        (not _cached_token)
        or (not _token_expires_at)
        or (now + timedelta(seconds=60) >= _token_expires_at)
    ):
...
def parse_styles_text(text):
    """Parse target styles and normalize entries to only these keys:
    offset(str), length(str), style. Drop any other keys."""
    if not text:
        return []
    try:
        raw = json.loads(text)
    except Exception:
        logging.warning("Invalid JSON in cococola_targetstylesN; returning empty list.")
        return []
    ...
    for item in raw:
        ...
        out = {}
        if "offset" in item:
            out["offset"] = str(item["offset"])
        if "length" in item:
            out["length"] = str(item["length"])
        if "style" in item:
...
def build_response(case_id):
    ...
    def make_translations(rec, prefix, src_prefix):
        """Build per-language translations with per-language styles."""
        out = []
        for i, lang in enumerate(target_languages, start=1):
            key = f"{prefix}{i}"
            src = src_prefix if i == 1 else f"{src_prefix}{i}"
            styles_col = f"cococola_targetstyles{i}"
            out.append(
                {
                    "language": lang,
                    "styles": parse_styles_text(rec.get(styles_col)),
                    "values": [{"value": rec.get(key), "source": rec.get(src)}],
                }
            )
        return out

    # 3. Sections (the four free-text sections carry per-language styles)
    sections = [
    ...
    base_fields = (
        ...
        + [f"cococola_targetstyles{i}" for i in range(1, 7)]
    )
                        ...
                        vals = [
                            {
                                ...
                                "value": full_trans,
                                "source": (
                                    rec["cococola_translationsource"]
                                    if i == 1
                                    else rec.get(f"cococola_translationsource{i}")
                                ),
                            }
                        ]
                    special.append(
                        {
                            "language": lang,
                            "styles": parse_styles_text(
                                rec.get(f"cococola_targetstyles{i}")
                            ),
    ...
    # 4. Nutrition Values (no styles here)
    nv_fields = (
        [
            "cococola_nutrientcomposition",
            "cococola_english",
            "cococola_nutrientid",
            "cococola_unit",
        ]
    ...
    nv_rows = fetch_records(
        NUTRITION_VALUES_TABLE,
        filter_clause=f"cococola_caseid eq '{case_id}'",
        ...
        units_trans = [
            {
                ...
                "values": [
                    {
                        "value": rec.get(f"cococola_unittranslationlang{i}"),
                        "source": rec.get(f"cococola_translationsourceunitlang{i}"),
                    }
                ],
            }
...
def main(req: HttpRequest) -> HttpResponse:
    ...
    if not case_id:
        return func.HttpResponse(
            "Please include 'caseId' in the request body.", status_code=400
        )
...
# def parse_styles_text(text):
#     """Dataverse stores *target* styles as multiline text containing JSON; parse safely."""
#     if not text:
#         return []
#     try:
#         return json.loads(text)
#     except Exception:
#         logging.warning("Invalid JSON in cococola_targetstyles; returning empty list.")
#         return []
...
#     # 3. Sections (free-text sections include target styles)
#     sections = [
#         ("Legal Name & Ingredient Information", LEGAL_TABLE),
#         ("Other Information", OTHER_INFO_TABLE),
#         ("Nutrition Information", NUTRITION_INFO_TABLE),
...
#         for rec in rows:
#             key = rec["cococola_key"]
#             styles = parse_styles_text(rec.get("cococola_targetstyles"))
...
#                         ]
#                     special.append(
#                         {"language": lang, "translation": full_trans, "values": vals}
#                     )
#                 item = {
#                     "id": rec["cococola_itemid"],
#                     "label": rec["cococola_label"],
