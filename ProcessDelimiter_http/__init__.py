...
def get_delimiters_for_language(language):
    ...
    try:
        ...
        records = fetch_records(token, "cococola_tccc_languagedelimitermapping", filter_clause=filter_q, select_clause=select_q)
...
def split_units(text, language, field_name):
    ...
    if not text or not field_name:
        logging.error("Text or field_name missing: text='%s', field_name='%s'", text, field_name)
        return []
...
def split_and_merge_texts_for_languages(text1, lang1, text2, lang2, field_name):
    ...
    if len(parts1) != len(parts2):
        logging.warning("Length mismatch for '%s': %d vs %d", field_name, len(parts1), len(parts2))
    ...
    try:
        if process == "TranslationStep1":
            ...
            if not text or not language or not field_name:
                ...
                return func.HttpResponse(json.dumps({"error": "Missing text, language, or fieldName."}), status_code=400, mimetype="application/json")
            ...
            if not main_str or not isinstance(src, list) or not isinstance(tgt, list):
                ...
                return func.HttpResponse(json.dumps({"error": "Missing main_string or arrays."}), status_code=400, mimetype="application/json")
            ...
            if not all([t1, l1, t2, l2, field_name]):
                ...
                return func.HttpResponse(json.dumps({"error": "Missing catalog parameters."}), status_code=400, mimetype="application/json")
            merged = split_and_merge_texts_for_languages(t1, l1, t2, l2, field_name)
            result = {"merged": merged}

        else:
            logging.error("Unsupported process_type '%s'", process)
            return func.HttpResponse(json.dumps({"error": "Unsupported process_type."}), status_code=400, mimetype="application/json")
